...
You can use the Agent-based Installer to deploy {sno} on bare-metal servers, including ARM (`aarch64`) architecture. The Agent-based Installer generates a self-contained bootable ISO image by using the {product-title} installer for offline and automated deployments.
